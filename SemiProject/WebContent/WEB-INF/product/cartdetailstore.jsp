...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$("button#jibul").click(function(){
			...
			// 포인트 변수에 담아두기
				...
				if(nampoint<0){
					...
					// 사용포인트 지우기
					$("td#pthis").text("");
					$("td#pthis2").html("<fmt:formatNumber value='${requestScope.sumoption_price}' pattern='###,###' />원");
					$("td#pthis3").html("<fmt:formatNumber value='${requestScope.sumsalePrice + sumoption_price}' pattern='###,###' />원");	
					// 저장한 값들 다 초기화
					jibulpoint = 0;
					dbpoint = 0;
					$("input#dbpoint").val("0");
				...
				// 성공적인 사용
					...
					// 부가옵션 - 내 포인트
					var minuspoint = "${requestScope.sumoption_price}"-jibulpoint;
					// 콤마 넣기
					var regexp = /\B(?=(\d{3})+(?!\d))/g;
					...
					// 우선-인지 아닌지 보기
					if(minuspoint<=0){
						finalpoint = "${requestScope.sumoption_price}" - jibulpoint;
						var regexp = /\B(?=(\d{3})+(?!\d))/g;
						var finpoint = finalpoint.toString().replace(regexp, ',');
						$("td#pthis3").text(finpoint+"원");
					}
					else{
						finalpoint = "${requestScope.sumsalePrice}"*1 + "${requestScope.sumoption_price}"*1 - jibulpoint;
						var regexp = /\B(?=(\d{3})+(?!\d))/g;
						var finpoint = finalpoint.toString().replace(regexp, ',');
						$("td#pthis3").text(finpoint+"원");
		...
		$("button#cancle").click(function(){
			...
			$("td#pthis").text("");
			$("td#pthis2").html("<fmt:formatNumber value='${requestScope.sumoption_price}' pattern='###,###' />원");
			$("td#pthis3").html("<fmt:formatNumber value='${requestScope.sum_totalprice}' pattern='###,###' />원");	
			// 저장한 값들 다 초기화
			jibulpoint = 0;
			dbpoint = 0;
			$("input#dbpoint").val("0");
		...
		if(flagname == true && flagpostcode == true && flagaddress == true && flagdetailAddress == true && flagh2 == true && flagh3 == true && flagfirstemail == true && flagsecondemail == true){
			...
			// 아임포트 결제 팝업창 띄우기 
	      	...
	      	test.addEventListener('beforeunload', function(){
				...
				frm.action = "<%= ctxPath%>/cartDetailBoughtAddDB.sh";
				frm.method = "post";
				frm.submit();
...
<div id="container" style="width:1200px; height:1600px; margin-left:200px; font-family:맑은 고딕;">
	 <form name="fordetailbought">
		<table style="margin-top:80px;">
			...
			<c:forEach var="ord" items="${requestScope.orderlist}" varStatus="status">
				<tr>						
					<td style="width:100px; height:150px; font-size:10pt; color:black; text-align:center;">
						<img style="height:150px; width:100px;" id="chun" src="${ord.get('pimage1')}"/>
						
						
					 		<input type="hidden" class="pimage1" name="pimage1" value="${ord.get('pimage1')}"/>
						 	<input type="hidden" class="pname" name="pname" value="${ord.get('pname')}"/>						
							<input type="hidden" class="pnum"  name="pnum" value="${ord.get('pnum')}"/>
							<input type="hidden" class="cartno" name="cartno" value="${ord.get('cartno')}"/>
							<input type="hidden" class="qty"  name="qty" value="${ord.get('qty')}"/>
							<input type="hidden" class="option" name="option" value="${ord.get('option')}"/>
							<input type="hidden" class="option_price" name="option_price" value="${ord.get('option_price')}"/>						
							<input type="hidden" class="salePrice" name="salePrice" value="${ord.get('salePrice')}"/>
							<input type="hidden" class="totalPoint" name="totalPoint" value="${ord.get('totalPoint')}"/>
							<input type="hidden" class="totalPrice" name="totalPrice" value="${ord.get('totalPrice')}"/> 
					...
					<td style="width:100px; height:150px; font-size:10pt; color:black; text-align:center;">
						<fmt:formatNumber value="${ord.get('salePrice')*ord.get('qty')}" pattern="###,###" />원
					</td>
					<td style="width:100px; height:150px; font-size:10pt; color:black; text-align:center;">
						<fmt:formatNumber value="${ord.get('option_price')*ord.get('qty')}" pattern="###,###" />원
					</td>
					<td style="width:100px; height:150px; font-size:10pt; color:black; text-align:center;">
						<fmt:formatNumber value="${ord.get('totalPoint')}" pattern="###,###" />원	
					</td>
					<td style="width:100px; height:150px; font-size:10pt; color:black; text-align:center;">
						[기본배송]<br>
						배송비 : 무료
					...
					<td style="width:100px; height:150px; font-size:10pt; color:black; text-align:center;">
						<fmt:formatNumber value="${ord.get('totalPrice')}" pattern="###,###" />원
					</td>
				</tr>
			</c:forEach>
					...
					<table style="width:1200px; height:240px; font-size:9.5pt; border:solid 1px #d0d0e2;">
						...
						<tr style="border:solid 1px #d0d0e2;">
							<td style="text-align:center; width:300px; height:80px; font-weight:bold; font-size:17pt;">
								<fmt:formatNumber value="${requestScope.sumsalePrice }" pattern="###,###" />원
							</td>
							...
							<td style="text-align:center; width:300px; height:80px; font-weight:bold; font-size:17pt; border:solid 1px #d0d0e2;">
								<fmt:formatNumber value="${requestScope.sumoption_price}" pattern="###,###" />원
							</td>
							...
							<td id="pthis3" style="text-align:center; width:300px; height:80px; font-weight:bold; font-size:17pt;">
								<fmt:formatNumber value="${requestScope.sum_totalprice}" pattern="###,###" />원
							</td>
						</tr>
						<tr style="border:solid 1px #d0d0e2;">
							...
							<td id="buynow" style="width:300px; height:40px; font-size:12pt; text-align:center; background-color:black; color:white; border:solid 1px black; cursor:pointer;" onclick="godetailbought();">
								BUY NOW
							</td>
						</tr>
						<tr style="border:solid 1px #d0d0e2;">	
						...
						<tr style="border:solid 1px #d0d0e2;">	
							<td colspan="4" style="text-align:center; height:40px; width:300px; border:solid 1px #d0d0e2;">
								<fmt:formatNumber value="${requestScope.sum_totalpoint}" pattern="###,###" />원
							</td>	
						</tr>
					</table>
				</td>
