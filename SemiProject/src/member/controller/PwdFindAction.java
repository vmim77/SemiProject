	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if("POST".equalsIgnoreCase(method)) {
			...
			//	System.out.println("~~~~ 확인용 certificationCode => " + certificationCode);
				// ~~~~ 확인용 certificationCode => awkfk7274003
				
				// 랜덤하게 생성한 인증코드(certificationCode)를 비밀번호 찾기를 하고자 하는 사용자의 email로 전송시킨다.
				/*
				 * GoogleMail mail = new GoogleMail();
				 * 
				 * try { mail.sendmail(email, certificationCode); sendMailSuccess = true; // 메일
				 * 전송 성공했음을 기록함.
				 * 
				 * // 세션불러오기 HttpSession session = request.getSession();
				 * session.setAttribute("certificationCode", certificationCode); // 발급한 인증코드를
				 * 세션에 저장시킴.
				 * 
				 * } catch(Exception e) { // 메일 전송이 실패한 경우 e.printStackTrace(); sendMailSuccess
				 * = false; // 메일 전송 실패했음을 기록함. }
				 */
